...
const OverlayCard: React.FC<Props> = ({ item, height = 'h-64' }) => (
    <Link href={item.sub_category ? `/${item.category?.slug}/${item.sub_category?.slug}/${item.slug}` : `/${item.category?.slug}/${item.slug}`} className={`group relative block overflow-hidden rounded-2xl ${height} w-full shadow-lg transition-all hover:shadow-2xl`}>
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 md:p-7">
            <span className="mb-3 inline-flex items-center rounded-full bg-primary px-3 py-1 text-xs font-bold uppercase tracking-wider text-primary-foreground shadow-xl">{item.category?.name || 'Berita'}</span>
            <h3 className="line-clamp-3 font-serif text-2xl font-bold leading-tight text-white transition-colors group-hover:text-primary-foreground sm:text-3xl md:text-3xl">{item.title}</h3>

        ...
        <div className="min-h-screen bg-background font-sans text-foreground transition-colors">
            ...
            <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                <div className="mb-10 border-b-2 border-[#0455A4] pb-6">
                    <h1 className="text-4xl font-extrabold text-[#0455A4] uppercase tracking-tight">{category.name}</h1>
                    <p className="mt-3 text-lg text-gray-500 dark:text-gray-400 max-w-2xl">{category.description}</p>
                </div>
                            ...
                            <div className="mb-12">
                                <Link href={`/${news.data[0].category.slug}/${news.data[0].slug}`} className="group block">
                                    <div className="aspect-[21/9] w-full overflow-hidden rounded-2xl bg-gray-100 dark:bg-gray-800">
                                        {news.data[0].thumbnail_url ? (
                                            <img src={news.data[0].thumbnail_url} alt={news.data[0].title} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
                                        ) : (
                                            <div className="flex h-full items-center justify-center text-gray-400 font-bold">No Image</div>
                                        )}
                                    </div>
                                    <div className="mt-6">
                                        <h2 className="text-3xl font-bold text-gray-900 group-hover:text-[#0455A4] transition-colors leading-tight dark:text-gray-100">{news.data[0].title}</h2>
                                        <p className="mt-4 text-gray-600 line-clamp-3 dark:text-gray-400">Liputan mendalam tentang {news.data[0].title} dan perkembangan terbaru di sektor {category.name}.</p>
                                    </div>
                                </Link>
                            </div>
                        )}
                            ...
                            {news.data.slice(1).length > 0 ? (
                                news.data.slice(1).map((item) => (
                                    <Link key={item.id} href={`/${item.category.slug}/${item.slug}`} className="group flex items-start gap-6 border-b border-gray-100 py-8 last:border-0 dark:border-gray-800">
                                        <div className="aspect-[16/9] w-[180px] flex-shrink-0 overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-900 md:w-[240px]">
                                        ...
                                        <div className="flex flex-col gap-3">
                                            <h2 className="line-clamp-2 text-xl font-bold text-gray-900 transition-colors group-hover:text-[#0455A4] md:text-2xl dark:text-gray-100 leading-tight tracking-tight">{item.title}</h2>

            ...
            <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:ml-20 lg:px-8">
                ...
                <div className="mb-12">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-4xl font-black tracking-tighter text-foreground uppercase italic sm:text-5xl">{category.name}</h1>
                    </div>
                    <p className="mt-4 max-w-2xl text-base font-medium text-muted-foreground">{category.description}</p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-12 lg:grid-cols-12">
                    ...
                    <div className="lg:col-span-8">
                        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2">
                            {news.data.length > 0 ? (
                                news.data.map((item) => {
                                    const itemCategorySlug = item.category?.slug || 'news';
                                    const href = `/${itemCategorySlug}/${item.slug}`;

                                    return (
                                        <Link key={item.id} href={href} className="group flex flex-col gap-4">
